...
- **A stalled device explains itself.** Some USB sticks stop responding for minutes and then recover. The progress bar now counts the silence, and past a minute the status says what is happening — including, before the first write, that the restore has not yet changed the disk; after the first write it says plainly to leave the stick plugged in.
